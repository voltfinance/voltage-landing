...
export const STABLESWAP_SUBGRAPH_URL =
  "https://api.thegraph.com/subgraphs/name/t0mcr8se/stableswap-subgraph"; // TODO: deploy under voltfinance
export const FUSESWAP_SUBGRAPH_URL =
...
export const VOLTAGE_SUBGRAPH_URL =
...
export const tryFormatDecimalAmount = (
  amount,
  tokenDecimals = 18,
  decimals = 0
) => {
...
`;
export const getStableswapTokenBalances = async (
        ...
        swap.balances.reduce((mem, rawBalance, i) => {
          ...
          return mem.add(
            new TokenAmount(token, rawBalance)
              .multiply(((usdPrices[token.address] ?? 1) * 1e18).toString())
              .toFixed(0)
          );
        }, BigNumber.from("0"))
      );
    }, BigNumber.from("0"));
    const totalLockedPlusSwap =
      totalLocked +
      parseFloat(tryFormatDecimalAmount(liquidity.toString(), 18, decimals));
